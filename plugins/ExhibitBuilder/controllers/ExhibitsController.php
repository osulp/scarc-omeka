...
class ExhibitBuilder_ExhibitsController extends Omeka_Controller_AbstractActionController
{
    ...
    public function tagsAction()
    {
        ...
        $this->renderExhibit(compact('exhibit'), 'tags');
    }

    # for the transcribe browse item in the ecas exhibit pages
    public function transcribebrowseAction()
    {
        #$params = array_merge($this->_getAllParams(), array('type'=>'Exhibit'));
        #$tags = $this->_helper->db->getTable('Tag')->findBy($params);
        #$this->view->assign(compact('tags'));
        $request = $this->getRequest();
        $alphaParam = $this->_getParam('alpha');
...
# $_GET['tags']
	...
	if (!$this->_getParam('sort_field')) {
        ...
        fire_plugin_hook('show_exhibit', array('exhibit' => $exhibit));
        $this->renderExhibit(compact('exhibit'), 'transcribebrowse');

    }

    public function itemsbrowseOLDAction()
    {
        #$params = array_merge($this->_getAllParams(), array('type'=>'Exhibit'));
        #$tags = $this->_helper->db->getTable('Tag')->findBy($params);
        #$this->view->assign(compact('tags'));
    ...
    public function itemsbrowse2Action()
    {
        $params = array_merge($this->_getAllParams(), array('type'=>'Exhibit'));
        $tags = $this->_helper->db->getTable('Tag')->findBy($params);
        $this->view->assign(compact('tags'));
...
#        if ($item && $exhibit->hasItem($item) ) {
    ...
    protected function renderExhibit($vars, $toRender = 'show')
    {
        ...
        if (!in_array($toRender, array('show', 'summary', 'item', 'itemsbrowse', 'tags', 'transcribebrowse'))) {
