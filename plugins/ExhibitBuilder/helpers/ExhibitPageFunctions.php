...
function exhibit_builder_page_nav($exhibitPage = null)
{
    if (!$exhibitPage) {
		...
		//$exhibitPage = $exhibit-getFirstTopPage();

        if (!($exhibitPage = get_current_record('exhibit_page', false))) {
			$exhibit = get_current_record('exhibit', false);
			$exhibitPages = $exhibit->getTopPages();

			$first = true;	
			foreach ($exhibitPages as $page) {
				if ( $first )  {
    ...
    $html = '<ul class="exhibit-page-nav" id="secondary-nav">' . "\n";
...
//    $html .= html_escape($exhibit->title) .'</a></li>' . "\n";
    foreach ($pagesTrail as $page) {
        ...
        foreach ($pageSiblings as $pageSibling) {
			...
			// Only display pages if top level, hide child pages
			if (!($pageSibling->getParent())) {

	            $html .= '<li' . (($pageSibling->id == $page->id and rtrim(current_url(), "/") != exhibit_builder_exhibit_uri()) ? ' class="current"' : '') . '>';
...
/**
 * Return the markup for the exhibit lower level page navigation.
 *
 * @param ExhibitPage|null $exhibitPage If null, uses the current exhibit page
 * @return string
 */

function exhibit_builder_lower_page_nav($exhibitPage = null)
{
    if (!$exhibitPage) {
        $exhibitPage = get_current_record('exhibit_page');
    }
	...
	$pageParent = $exhibitPage->getParent();

    $children = exhibit_builder_child_pages($pageParent);

    $html = '';
    // check if there is actually a page parent
    if ($pageParent) {
        $html = '<h2>' . html_escape($pageParent->title) . '</h2>' . "\n";
    }

    $html .= '<ul class="exhibit-child-nav">' . "\n";
    foreach ($children as $child) {
        $html .= '<li' . ($exhibitPage->id == $child->id ? ' class="current"' : '') . '>';	
		$html .= '<a href="' . html_escape(exhibit_builder_exhibit_uri($exhibit, $child)) . '">' . html_escape($child->title) . '</a></li>' . "\n";
    }
    $html .= '</ul>' . "\n";
...
function exhibit_builder_link_to_next_page($text = null, $props = array(), $exhibitPage = null)
{
    ...
    if (metadata($exhibit, 'id') == 86 and $nextPage2 = $exhibitPage->firstChildOrNext()) {
                if ($nextPage2->getParent()) {
			$targetPage2 = $nextPage2;
		}
		else {
//			$targetPage2 = $nextPage2->firstChildOrNext();
			$targetPage2 = $nextPage2;
		}


//		echo 'TEST: ' . metadata($targetPage2, 'title') . ' (' . metadata($targetPage2, 'slug') . ')';
	}
    ...
    if ($nextPage = $exhibitPage->firstChildOrNext()) {
		if ($nextPage->getParent()) {
			$targetPage = $nextPage;
		}
		else {
			$targetPage = $nextPage->firstChildOrNext();
		}
...
function exhibit_builder_link_to_previous_page($text = null, $props = array(), $exhibitPage = null)
{
    ...
    if ($previousPage = $exhibitPage->previousOrParent()) {
		if ($previousPage->getParent()) {
			$targetPage = $previousPage;
		}
		else {
			$targetPage = $previousPage->previousOrParent();
		}
	}
	...
	if ($targetPage) {
        ...
        if ($text === null) {
            $text = '&larr; ' . metadata($targetPage, 'title');
        }
        return exhibit_builder_link_to_exhibit($exhibit, $text, $props, $targetPage);
    }
